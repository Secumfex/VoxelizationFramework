...
/** \brief A collection of useful global functions to reduce the code for shader complation
 *
 */
namespace ShaderTools {
	/**
	 * Checks a shader for compilation errors
	 * @param shaderHandle The id of the shader to check
	 */
	void checkShader(GLuint shaderHandle);

	/**
	 * Reads a file and returns the content as a pointer to chars
	 * @param shaderHandle The id of a created the shader
	 * @param fileName The path and filename to a shader file.
	 ...
	 * @param vertexShaderName The path and filename to a vertex shader file.
	 * \n Recommendation: Define compiler flag @c -D"SHADERS_PATH" and call 
	 * @a SHADERS_PATH"[filename]"
	 * @param fragmentShaderName The path and filename to a fragment shader file.
