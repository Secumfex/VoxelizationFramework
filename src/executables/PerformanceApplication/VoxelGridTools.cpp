...
PrintVoxelGridInfoListener::PrintVoxelGridInfoListener(
		VoxelGridGPU* voxelgridGPU) {
	p_voxelgridGPU = voxelgridGPU;
}

void PrintVoxelGridInfoListener::call() {
	if ( p_voxelgridGPU != 0)
	{
		p_voxelgridGPU->printInfo();
	}
}

Listener* VoxelGridGPU::getPrintVoxelGridInfoListener() {
return new PrintVoxelGridInfoListener(this);
}
